...
contract Game{
	...
	// Stage 0: Create
	// Initialize
	function Game(uint _betTimePeriodInMinutes, address _humonOracle) {
		bettingTime = now + _betTimePeriodInMinutes * 1 minutes;
		humonOracle = _humonOracle;
		state = State.Betting;
	}
	...
	function revealResult(uint _winner) onlyAt(State.GettingResult){
		if(msg.sender != humonOracle) throw;
		winner = _winner % 2;
